...
const adminRoleCookie = useCookie('admin_role', sessionCookieOptions)
...
async function handleLogin() {
  ...
  try {
    ...
    adminRoleCookie.value = data.usuario?.rol || data.rol || 'USER'

    setTimeout(() => {
      emit('loginSuccess', { adminId: data.adminId, nombre: data.nombre, rol: data.usuario?.rol || data.rol || 'USER' })
      resetForm()
    }, 1200)

      ...
      <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <header class="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-20 px-6 py-4 flex items-center justify-between">
          ...
          <div class="flex items-center space-x-4">
            ...
            <!-- Botón Ir a Inicio (Sitio Público) -->
            <NuxtLink
              to="/"
              class="inline-flex items-center justify-center px-3.5 py-2 rounded-xl text-xs font-black text-slate-700 bg-slate-100 hover:bg-slate-200 transition-all border border-slate-200"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3.5 h-3.5 mr-1 text-[#0b31a8]">
...
function checkAdminRole() {
  if (!tokenCookie.value) {
    ...
    addToast('Debes iniciar sesión para acceder al panel admin.', 'warning')
  } else if (adminRoleCookie.value !== 'ADMIN') {
    navigateTo('/')
...
watch([tokenCookie, adminRoleCookie], () => {
  checkAdminRole()
})
...
const navItems = [
  ...
  },
  {
    key: 'tematicas',
    label: 'Temáticas',
    path: '/admin/tematicas',
    icon: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v13.5A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V9.75m-9.75-6.75h5.25m-.001 0L21 6m0 0H16.5V1.5" /></svg>`
  },
  {
    key: 'usuarios',
    label: 'Voluntarios',
